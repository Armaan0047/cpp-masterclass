...
// ─────────────────────────────────────────────────────────────────────────────
// FIXED SECTION 1 — Modular Firebase imports (replaces old firebaseConfig block)
// ─────────────────────────────────────────────────────────────────────────────
import { auth, db } from "./firebase.js";
import { cppCurriculum } from "./data.js";
import {
    GoogleAuthProvider,
    signInWithPopup,
    onAuthStateChanged,
...
// ─────────────────────────────────────────────────────────────────────────────
// FIXED SECTION 2 — Authentication (replaces entire old handleAuth block)
// ─────────────────────────────────────────────────────────────────────────────
function handleAuth() {
    const loginBtn    = document.getElementById('login-btn');
    const userProfile = document.getElementById('user-profile');
    const userInitials = document.getElementById('user-initials');
    const userAvatar  = document.getElementById('user-avatar');   // optional avatar img
    const welcomeMsg  = document.getElementById('welcome-message');

    // ── Reactively update UI whenever auth state changes (persists on refresh) ──
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            // Initials fallback
            userInitials.innerText = user.displayName
                ? user.displayName.split(' ').map(n => n[0]).join('').toUpperCase()
                : 'U';

            // Show photo avatar if element exists and photo is available
            ...
            welcomeMsg.innerText = `Welcome, ${user.displayName ? user.displayName.split(' ')[0] : 'User'}!`;
...
// ─────────────────────────────────────────────────────────────────────────────
// FIXED SECTION 3 — Firestore sync (replaces old compat-style Firestore calls)
// ─────────────────────────────────────────────────────────────────────────────
async function syncProgressFromCloud() {
    ...
    try {
        ...
        console.warn("Could not sync from cloud:", e.message);
    }
}
...
async function syncProgressToCloud() {
    ...
    try {
        ...
        console.warn("Could not sync to cloud:", e.message);
    }
}
// ─────────────────────────────────────────────────────────────────────────────
